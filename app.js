...
/* ═══════════════════════════════════════════════
   AI Description Assistant (OpenAI ChatGPT)
════════════════════════════════════════════════ */
const aiSuggestBtn = $('ai-suggest-btn');

/* Show the AI button only when both issue type AND severity are selected */
function updateAiButtonVisibility() {
  if (!window.__AI_ENABLED__) return;
  const hasType = !!issueTypeEl.value;
  const hasSev  = !!document.querySelector('input[name="severity"]:checked');
  aiSuggestBtn.hidden = !(hasType && hasSev);
}
issueTypeEl.addEventListener('change', updateAiButtonVisibility);
document.querySelectorAll('input[name="severity"]').forEach(r =>
  r.addEventListener('change', updateAiButtonVisibility)
);
...
async function suggestDescription() {
  ...
  const address   = locationDisplay.textContent.replace('⏳ Fetching address…', '').trim() || 'Bangalore';

  /* Button → loading state */
  const label   = aiSuggestBtn.querySelector('.ai-suggest-label');
  const spinner = aiSuggestBtn.querySelector('.ai-suggest-spinner');
  const icon    = aiSuggestBtn.querySelector('.ai-suggest-icon');
  ...
  try {
    /* Call Firebase Cloud Function proxy via Hosting rewrite — no API key in browser */
    const res = await fetch('/api/suggest', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ issueType, severity, address }),
    });
    ...
    /* Populate textarea, flash border, update char count */
    descriptionEl.value         = text.slice(0, 300);
    charCountEl.textContent     = `${descriptionEl.value.length} / 300`;
    descriptionEl.classList.remove('error');
    descriptionEl.classList.add('ai-filled');
    setTimeout(() => descriptionEl.classList.remove('ai-filled'), 1800);
    $('description-error').textContent = '';
    showToast('✨ AI description generated');
  } catch (err) {
    console.error('[RoadWatch] OpenAI error:', err);
...
aiSuggestBtn.addEventListener('click', suggestDescription);
